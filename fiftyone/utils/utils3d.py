...
def compute_orthographic_projection_maps(
    ...
    This operation will add ``orthographic_projection_path`` and
    ``orthographic_projection_params`` fields to each sample.
    ...
    if out_group_slice is not None:
        ...
        out_image_field = "orthographic_projection_path"

    out_params_field = "orthographic_projection_params"
    ...
    with fou.ProgressBar(total=len(filepaths)) as pb:
        for filepath, group in pb(zip(filepaths, groups)):
            ...
            if out_group_slice is not None:
                ...
                sample[out_params_field] = [width, height, *map_bounds]
                ...
                all_bounds.append([width, height, *map_bounds])

    # add width and height to projection parameters
    point_cloud_view.set_values(out_params_field, all_bounds)
